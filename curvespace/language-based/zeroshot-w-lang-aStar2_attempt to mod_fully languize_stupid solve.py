...
while("SOLVED" not in answer):
    # removing state, and just describing directly: losing state loses some logic? ignoring state misses task
    answer = ai("describe what was done so far, and prediction for how much work is left in ideal scenario.", [task, answer, "using language", language])
